...
void processAutoBox(const PxF32 timestep, PxVehicle4W& car)
{
	//Process the autobox.
	if(car.mUseAutoGears)
	{
		...
		const PxVehicleAutoBoxData& autoBoxData=carData.getAutoBoxData();

		PxF32 autoBoxSwitchTime=car.mAutoBoxSwitchTime;
		const PxF32 autoBoxLatencyTime=autoBoxData.mDownRatios[PxVehicleGearsData::eREVERSE];

		const PxF32 normalisedEngineOmega=car.mInternalDynamics[PxVehicle4WSimulationData::eNUM_WHEELS]*engineData.getRecipMaxOmega();
		...
		//If in neutral gear because we're undergoing a gear change then turn off accel pedal.
		if(PxVehicleGearsData::eNEUTRAL==currentGear && currentGear!=targetGear)
		{
		...
		if(normalisedEngineOmega > autoBoxData.mUpRatios[currentGear] && currentGear==targetGear && PxVehicleGearsData::eREVERSE!=currentGear)
		{
			gearUpPressed=true;
		}

		//If revs too low and in gear greater than first and not undergoing a gear change then change down.
		if(normalisedEngineOmega < autoBoxData.mDownRatios[currentGear] && currentGear==targetGear && currentGear > PxVehicleGearsData::eFIRST)
		{
			gearDownPressed=true;
		}

		const PxF32 forwardSpeed=PxVehicle4WComputeForwardSpeed(car);

		//Toggle between reverse and first gears.
		if(!gearUpPressed && !gearDownPressed && currentGear==targetGear && (PxVehicleGearsData::eFIRST==currentGear || PxVehicleGearsData::eREVERSE==currentGear))
		{
			if(PxVehicleGearsData::eFIRST==currentGear && car.mGearDownPressed && forwardSpeed<gMinSpeedForReverseGearAutoBoxActiveToggle)
			{
				//Switch based on user pressing geardown and car having small forward speed.
				gearDownPressed=true;
			}
			else if(PxVehicleGearsData::eREVERSE==currentGear && car.mGearUpPressed && forwardSpeed>-gMinSpeedForReverseGearAutoBoxActiveToggle)
			{
				//Switch based on user pressing gearup and car having small backwards speed.
				gearUpPressed=true;
			}
		}

		//Toggle between first/neutral and reverse/neutral
		if(!gearUpPressed && !gearDownPressed && currentGear==targetGear && (PxVehicleGearsData::eFIRST==currentGear || PxVehicleGearsData::eREVERSE==currentGear) && 
			0.0f==(accel+brake+handbrake))
		{
			if(PxVehicleGearsData::eFIRST==currentGear && forwardSpeed<-gMinSpeedForReverseGearAutoBoxPassiveToggle)
			{
				//Switch based on the car having velocity in the wrong direction
				gearDownPressed=true;
				car.mTargetGear=PxVehicleGearsData::eNEUTRAL;
				autoBoxSwitchTime=0.75f*autoBoxLatencyTime;
			}
			else if(PxVehicleGearsData::eREVERSE==currentGear && forwardSpeed>gMinSpeedForReverseGearAutoBoxPassiveToggle)
			{
				//Switch based on the car having velocity in the wrong direction
				gearUpPressed=true;
				car.mTargetGear=PxVehicleGearsData::eNEUTRAL;
		...
		if(autoBoxSwitchTime>=autoBoxLatencyTime && (gearDownPressed || gearUpPressed))
		{
			car.mGearUpPressed=gearUpPressed;
			car.mGearDownPressed=gearDownPressed;
			autoBoxSwitchTime=0.0f;
		}
		else
		{
 ...
 PxF32* PX_RESTRICT tyreTorques, PxVec3& chassisForce, PxVec3& chassisTorque)
{
	...
	for(PxU32 i=0;i<PxVehicle4WSimulationData::eNUM_WHEELS;i++)
	{
		...
		{
			PX_ASSERT(hits[k].actor!=car.mActor);
		...
		if(mink!=0xffffffff)
		{
			...
			if(dx > -susp.mMaxDroop)
			{
				...
				PxRigidDynamic* dynamicHitActor=hits[mink].actor->isRigidDynamic();
				if(dynamicHitActor)
				{
 ...
 PxVehicle4W& car)
{
	...
	for(PxU32 k=0;k<numSubSteps;k++)
	{
		...
		PxF32 internalTorques[PxVehicle4WSimulationData::eNUM_WHEELS+1];
		for(PxU32 j=0;j<PxVehicle4WSimulationData::eNUM_WHEELS;j++)
		{
			internalTorques[j]=0;
		}
...
void PxVehicle4WUpdateSingleVehicleAndStoreTelemetryData(const PxF32 timestep, const PxVec3& gravity, const PxVehicleDrivableSurfaceToTyreFrictionPairs& vehicleDrivableSurfaceToTyreFrictionPairs, PxVehicle4W* focusVehicle, PxVehicle4WTelemetryData& telemetryData)
{
...
void PxVehicle4WUpdate(const PxF32 timestep, const PxVec3& gravity, const PxVehicleDrivableSurfaceToTyreFrictionPairs& vehicleDrivableSurfaceToTyreFrictionPairs, const PxU32 numVehicles, PxVehicle4W** vehicles)
{
	PX_CHECK_MSG(gravity.magnitude()>0, "gravity vector must have non-zero length");
	PX_CHECK_MSG(timestep>0, "timestep must be greater than zero");
	PX_CHECK_MSG(0==((size_t)vehicleDrivableSurfaceToTyreFrictionPairs.mPairs & 0x0f), "mVehicleDrivableSurfaceToTyreFrictionPairs must be 16-byte aligned");
