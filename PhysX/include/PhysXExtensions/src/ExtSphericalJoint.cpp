										 ...
										 PxRigidActor* actor1, const PxTransform& localFrame1)
{
	...
	return createJoint<PxSphericalJoint, PxSphericalJointDesc>(physics, actor0, localFrame0, actor1, localFrame1);
}
							 ...
							 PxU32 flags)
{
	...
	if(data.jointFlags & PxSphericalJointFlag::eLIMIT_ENABLED)
	{
		...
		visualizeLimitCone(r, limitScale, cA2w, data.tanQZLimit, data.tanQYLimit, 
...
void Ext::SphericalJoint::finishDesc(PxConstraintDesc& desc)
{
	desc.solverPrep = SphericalJointSolverPrep;
#ifdef PX_PS3
	setSphericalJointSolverPrepSpu(&desc);
#endif
	desc.project = SphericalJointProject;
	desc.visualize = SphericalJointVisualize;
	desc.dataSize = sizeof(SphericalJointData);
}
...
bool SphericalJoint::resolvePointers(PxRefResolver& v, void*)
{
	PxConstraintDesc desc;
#ifdef PX_PS3
	setSphericalJointSolverPrepSpu(&desc);
#endif
	setShader(resolveConstraintPtr(v, getShader(), getConnector(), SphericalJointSolverPrep, /*desc.solverPrepSpu*/NULL, /*desc.solverPrepSpuByteSize*/0, SphericalJointProject, SphericalJointVisualize));
	return true;
}
