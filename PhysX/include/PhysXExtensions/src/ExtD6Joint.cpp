						   ...
						   PxRigidActor* actor1, const PxTransform& localFrame1)
{
	...
	return createJoint<PxD6Joint, PxD6JointDesc>(physics, actor0, localFrame0, actor1, localFrame1);
}



D6Joint::D6Joint(const PxD6JointDesc& desc):
	D6JointT(desc),
	...
	mRecomputeLimits(true)
{
	...
	for(PxU32 i=0;i<6;i++)
		data->motion[i] = desc.motion[i];

	data->linearLimit = desc.linearLimit;
	data->twistLimit = desc.twistLimit;
	data->swingLimit = desc.swingLimit;

	for(PxU32 i=0;i<PxD6Drive::eCOUNT;i++)
		data->drive[i] = desc.drive[i];

	data->drivePosition = desc.drivePosition;
	data->driveLinearVelocity = desc.driveLinearVelocity;
	data->driveAngularVelocity = desc.driveAngularVelocity;
					  ...
					  PxU32 flags)
{
	...
	visualizeFrames(r, frameScale, cA2w, cB2w);
	...
	if(data.limited&TWIST_FLAG)
	{
		...
		visualizeAngularLimit(r, limitScale, cA2w, data.twistLimit.lower, data.twistLimit.upper, 
	...
	if(swing1Limited && swing2Limited)
	{
		...
		visualizeLimitCone(r, limitScale, cA2w, data.tqSwingZ, data.tqSwingY, 
	...
	else if(swing1Limited ^ swing2Limited)
	{
		...
		if(swing1Limited)
		{
			if(data.locked & SWING2_FLAG)
				visualizeAngularLimit(r, limitScale, cA2w * yToX, -data.swingLimit.yAngle, data.swingLimit.yAngle, 
					PxAbs(Ps::tanHalf(swing.y, swing.w)) > data.tqSwingY - data.tqSwingPad);
			else
				visualizeDoubleCone(r, limitScale, cA2w * zToX, data.swingLimit.yAngle, 
					PxAbs(tanHalfFromSin(aZ.dot(bX)))> data.thSwingY - data.thSwingPad);
		}
		else 
		{
			if(data.locked & SWING1_FLAG)
				visualizeAngularLimit(r, limitScale, cA2w * zToX, -data.swingLimit.zAngle, data.swingLimit.zAngle,
					PxAbs(Ps::tanHalf(swing.z, swing.w)) > data.tqSwingZ - data.tqSwingPad);
			else
				visualizeDoubleCone(r, limitScale, cA2w * yToX, data.swingLimit.zAngle,  
...
void D6Joint::finishDesc(PxConstraintDesc& desc)
{
	desc.solverPrep = D6JointSolverPrep;
#ifdef PX_PS3
	setD6JointSolverPrepSpu(&desc);
#endif
	desc.project = D6JointProject;
	desc.visualize = D6JointVisualize;
	desc.dataSize = sizeof(D6JointData);
}
...
bool D6Joint::resolvePointers(PxRefResolver& v, void*)
{
	PxConstraintDesc desc;
#ifdef PX_PS3
	setD6JointSolverPrepSpu(&desc);
#endif
	setShader(resolveConstraintPtr(v, getShader(), getConnector(),D6JointSolverPrep, /*desc.solverPrepSpu*/NULL, /*desc.solverPrepSpuByteSize*/0, D6JointProject, D6JointVisualize));
	return true;
}
