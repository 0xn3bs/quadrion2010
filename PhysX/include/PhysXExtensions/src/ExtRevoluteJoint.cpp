									   ...
									   PxRigidActor* actor1, const PxTransform& localFrame1)
{
	...
	return createJoint<PxRevoluteJoint, PxRevoluteJointDesc>(physics, actor0, localFrame0, actor1, localFrame1);
}
							...
							PxU32 flags)
{
	...
	if(data.jointFlags & PxRevoluteJointFlag::eLIMIT_ENABLED)
		joint::visualizeAngularLimit(r, limitScale, t0, data.limit.lower, data.limit.upper, false);
}
}

void Ext::RevoluteJoint::finishDesc(PxConstraintDesc& desc)
{
	desc.solverPrep = RevoluteJointSolverPrep;
#ifdef PX_PS3
	setRevoluteJointSolverPrepSpu(&desc);
#endif
	desc.project = RevoluteJointProject;
	desc.visualize = RevoluteJointVisualize;
	desc.dataSize = sizeof(RevoluteJointData);
}
...
bool RevoluteJoint::resolvePointers(PxRefResolver& v, void*)
{
	PxConstraintDesc desc;
#ifdef PX_PS3
	setRevoluteJointSolverPrepSpu(&desc);
#endif
	setShader(resolveConstraintPtr(v, getShader(), getConnector(), RevoluteJointSolverPrep, /*desc.solverPrepSpu*/NULL, /*desc.solverPrepSpuByteSize*/0, RevoluteJointProject, RevoluteJointVisualize));
	return true;
}
