...
#include "deprecated/PxRevoluteJointDesc.h"
...
//~PX_SERIALIZATION
		...
		static ConstraintInternalInterface* create(const PxRevoluteJointDesc& desc)
		{
			return PX_NEW(RevoluteJoint)(desc);
		}

		RevoluteJoint(const PxRevoluteJointDesc& desc): RevoluteJointT(desc)
		 {
// PX_SERIALIZATION
...
//~PX_SERIALIZATION
			...
			data->driveForceLimit = desc.driveForceLimit;
			data->driveVelocity = desc.driveVelocity;
			data->driveGearRatio = desc.driveGearRatio;
			data->jointFlags = desc.revoluteJointFlags;
		 }
