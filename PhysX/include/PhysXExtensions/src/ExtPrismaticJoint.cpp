										 ...
										 PxRigidActor* actor1, const PxTransform& localFrame1)
{
	...
	return createJoint<PxPrismaticJoint, PxPrismaticJointDesc>(physics, actor0, localFrame0, actor1, localFrame1);
}
							 ...
							 PxU32 flags)
{
	...
	{
		joint::visualizeLinearLimit(r, limitScale, t0, t1, data.limit.lower, ordinate < data.limit.lower + data.limit.contactDistance);
		joint::visualizeLinearLimit(r, limitScale, t0, t1, data.limit.upper, ordinate > data.limit.upper - data.limit.contactDistance);
	}
...
void Ext::PrismaticJoint::finishDesc(PxConstraintDesc& desc)
{
	desc.solverPrep = PrismaticJointSolverPrep;
#ifdef PX_PS3
	setPrismaticJointSolverPrepSpu(&desc);
#endif
	desc.project = PrismaticJointProject;
	desc.visualize = PrismaticJointVisualize;
	desc.dataSize = sizeof(PrismaticJointData);
}
...
bool PrismaticJoint::resolvePointers(PxRefResolver& v, void*)
{
	PxConstraintDesc desc;
#ifdef PX_PS3
	setPrismaticJointSolverPrepSpu(&desc);
#endif
	setShader(resolveConstraintPtr(v, getShader(), getConnector(), PrismaticJointSolverPrep, /*desc.solverPrepSpu*/NULL, /*desc.solverPrepSpuByteSize*/0, PrismaticJointProject, PrismaticJointVisualize));
	return true;
}
