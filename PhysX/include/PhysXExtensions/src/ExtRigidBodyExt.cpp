...
PxVec3 PxRigidBodyExt::getPointVelocity(PxRigidBody& body, const PxVec3& point)
{
	const PxVec3 rpoint = point - body.getGlobalPose().transform(body.getCMassLocalPose().p);

	PxVec3 velocity = body.getLinearVelocity();
	velocity += body.getAngularVelocity().cross(rpoint);
