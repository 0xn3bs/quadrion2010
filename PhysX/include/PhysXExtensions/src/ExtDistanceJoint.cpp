									   ...
									   PxRigidActor* actor1, const PxTransform& localFrame1)
{
	...
	return createJoint<PxDistanceJoint, PxDistanceJointDesc>(physics, actor0, localFrame0, actor1, localFrame1);
}
...
void Ext::DistanceJoint::finishDesc(PxConstraintDesc& desc)
{
	desc.solverPrep = DistanceJointSolverPrep;
	desc.project = DistanceJointProject;
#ifdef PX_PS3
	setDistanceJointSolverPrepSpu(&desc);
#endif
	desc.visualize = DistanceJointVisualize;
	desc.dataSize = sizeof(DistanceJointData);
}
...
bool DistanceJoint::resolvePointers(PxRefResolver& v, void*)
{
	PxConstraintDesc desc;
#ifdef PX_PS3
	setDistanceJointSolverPrepSpu(&desc);
#endif
	setShader(resolveConstraintPtr(v, getShader(), getConnector(), DistanceJointSolverPrep, /*desc.solverPrepSpu*/NULL, /*desc.solverPrepSpuByteSize*/0, DistanceJointProject, DistanceJointVisualize));
	return true;
}
